...
SELECT * FROM allpatients
  WHERE exclusion_pf=0 AND exclusion_vent=0 AND exclusion_los=0 AND exclusion_first_stay=0 AND exclusion_chest=0 AND exclusion_age=0
    ...
    SELECT ie.subject_id,ie.hadm_id,ie.icustay_id
      ...
      CASE
        WHEN valuenum > 0 THEN valuenum
        WHEN itemid IN (220277, 646) AND valuenum < 100 THEN valuenum -- spo2
        WHEN itemid IN (220224, 779) AND valuenum < 800 THEN valuenum -- pao2
        WHEN itemid IN (211, 220045) AND valuenum<300 THEN valuenum --hr
        WHEN itemid = 223835--fio2
          THEN CASE
               ...
               ELSE NULL END
        WHEN itemid IN (3420, 3422) AND valuenum < 100 THEN valuenum
        ...
        WHEN itemid IN (223761, 678, 679) AND valuenum<120 AND valuenum>70 THEN (valuenum - 32) * 5 / 9 --fahrenheit
        WHEN itemid IN (51, 442, 455, 6701, 220179, 220050) AND valuenum<400 THEN valuenum--bps
        WHEN itemid IN (8368, 8440, 8441, 8555, 220180, 220051) AND valuenum<300 THEN valuenum--bpd
        WHEN itemid IN (456, 52, 6702, 443, 220052, 220181, 225312) AND valuenum<300 THEN valuenum--bpm
        WHEN itemid IN (615, 618, 220210, 224690) AND valuenum<70 THEN valuenum--rr
